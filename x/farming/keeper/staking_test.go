		...
		suite.Run(tc.name, func() {
			...
			if tc.expectErr {
				...
				if suite.NoError(err) {
					suite.True(coinsEq(tc.remainingStaked, suite.keeper.GetAllStakingsByFarmer(suite.ctx, suite.addrs[tc.addrIdx])))
					suite.True(coinsEq(tc.remainingQueued, suite.keeper.GetAllQueuedStakingsByFarmer(suite.ctx, suite.addrs[tc.addrIdx])))
				}
			}
		})
	}
...
func (suite *KeeperTestSuite) TestProcessQueuedCoins() {
	for seed := int64(0); seed < 10; seed++ {
		...
		for i := 0; i < iterations; i++ {
			...
			suite.Require().True(coinsEq(queuedCoins, suite.keeper.GetAllQueuedStakingsByFarmer(suite.ctx, suite.addrs[0])))
			suite.Require().True(coinsEq(stakedCoins, suite.keeper.GetAllStakingsByFarmer(suite.ctx, suite.addrs[0])))
